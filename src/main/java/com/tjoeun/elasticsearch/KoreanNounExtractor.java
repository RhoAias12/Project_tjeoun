...
public class KoreanNounExtractor {
    ...
    public static Set<String> extractNouns(String text) {
        ...
        return OpenKoreanTextProcessorJava.tokensToJavaKoreanTokenList(tokens).stream()
                ...
                .map(token -> token.getText().toString().trim()) // 공백 제거
                .peek(noun -> System.out.println("추출된 명사: " + noun)) // 로그로 명사 확인
                .filter(noun -> noun.length() > 1)  // 1글자 단어 필터
                .filter(noun -> {
                    String cleanedNoun = noun.replaceAll("[^a-zA-Z가-힣]", "").toLowerCase(); // 특수 문자 제거 및 소문자 처리
                    System.out.println("🧹 깨끗한 명사: " + cleanedNoun); // 정리된 명사 출력
                    boolean isStopword = STOPWORDS.contains(cleanedNoun);
                    if (isStopword) {
                        System.out.println("🚫 제거된 불용어: " + noun);
                    }
                    return !isStopword;
                })
